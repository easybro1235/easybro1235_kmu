...
void loop() { 
  ...
  if ((distance < _DIST_MIN) || (distance > _DIST_MAX)) { //out of range
    analogWrite(PIN_LED, 255);       // LED OFF 
  }
  else {
    if(distance <= 200) {
      int new_dist = map(distance, 100, 200, 255, 0);
      analogWrite(PIN_LED, new_dist);
    }
    else if(distance > 200) {
      int new_dist = map(distance, 200, 300, 0, 255);
      analogWrite(PIN_LED, new_dist);
    }
  }  
